...
async def get_similar(
    text: str, r: NLPGetFilteredNodes, tx: Transaction = Depends(neodbsession.get_db)
):
    ...
    res = [
        {node_key: v[node_key], "score": nlptext.similarity(v["nlp"]), "lemmas": k}
        for k, v in cache_texts[r_filter].items()
    ]
    res = list(sorted(res, key=lambda x: x["score"], reverse=True))
    print("score list", res, cache_texts)
    res = list(filter(lambda x: x["score"] > min_score, res))
    res = [
        {node_key: item, "score": sublist["score"], "lemmas": sublist["lemmas"]}
        for sublist in res
        for item in sublist[node_key]
    ]
